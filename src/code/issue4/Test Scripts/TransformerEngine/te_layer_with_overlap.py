...
def _parse_args(argv=None, namespace=None):
    ...
    parser.add_argument(
        "-n", "--num-heads", type=int, default=96, help="Number of attention heads."
    )
    parser.add_argument(
...
def _train(opts):
  ...
  ##########
    WARMUP_ITERS = 3
    dist_print("Starting warmup iterations...")
  ...
  ##########
    ...
    iter_times = torch.zeros(opts.num_iters, device="cuda")
  ##########
  ...
  ##########
        ...
        dist_print(f"    Iter {i+1}/{opts.num_iters} - Time: {iter_time:.2f} ms", 
                  group=tp_group, debug=True)
        
    total_time = iter_times.sum()
    avg_time = total_time / opts.num_iters
    
    # 收集所有rank的时间（确保时间测量一致）
    time_data = torch.tensor([avg_time], device="cuda")
    dist.all_reduce(time_data, op=dist.ReduceOp.SUM)
    avg_time = time_data.item() / WORLD_SIZE
    
    # 输出结果（只在rank0打印）
    if WORLD_RANK == 0:
        print(f"\n{'='*50}")
        print(f"Training completed with {opts.num_iters} iterations")
        print(f"Average iteration time: {avg_time:.2f} ms")
        print(f"Throughput: {1000/avg_time:.2f} iter/s")
        print(f"{'='*50}\n")
  ##########

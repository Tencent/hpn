...
Kernel会获取QP、DCT、lkey、raddr、rkey等RDMA通信必须的元素，同时判断数据能否一次性传输。

该步骤是一个固定的、较小的软件开销。

2. 执行阶段（ $T_{exec}$ ）

Kernel调用ibgda_reserve_wqe_slots预留wqe空间，ibgda_write_rdma_write_wqe向网卡的指令队列写入RDMA WRITE指令，最终调用ibgda_submit_requests通知网卡开始执行数据传输。这一步骤是GPU为了发起通信而被占用的时间，耗时主要是多次的MMIO写入，由GPU向网卡直接写wqe。

在RDMA WRITE的wqe写入完成后，还会追加一个信令提交，表明传输完成。其与数据传输的提交是串行执行的，总耗时为两者之和。

3. 等待阶段（ $T_{wait}$ ）

如果Kernel是阻塞调用，会在最后执行ibgda_quiet，该函数读取最后一个WQE的编号prod_idx，并进入自旋等待，不断轮询完成队列CQ，直到获取目标prod_idx编号的CQE，意味着该任务已经被硬件执行完毕。此时ibgda_quiet等待结束，函数返回。

# 小结

|            | IBRC(CPU Proxy)                                           | IBGDA                                   |
| ---------- | --------------------------------------------------------- | --------------------------------------- |
| 实现       | GPU 写指令，CPU Proxy根据指令发起通信                     | GPU直接编程，与网卡直接通信             |
| 通信路径   | GPU → 指令队列(CPU内存) → CPU代理 → 网络库 → 网卡         | GPU → WQE队列(网卡内存) → 网卡          |
| 小消息延迟 | 高。软件栈长，且受限于CPU串行处理能力和GPU与CPU通信的开销 | 低。软件开销很小，直接控制网卡          |
| 大消息延迟 | 低。由于数据的增大，瓶颈主要在网络带宽，两者相差不大      | 低。但由于GPU的并行能力，更容易打满带宽 |
